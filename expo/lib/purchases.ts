...
// Use environment variable for RevenueCat API key. In production, ensure EXPO_PUBLIC_REVENUECAT_API_KEY is set.
// Falls back to test key for development only.
const API_KEY = process.env.EXPO_PUBLIC_REVENUECAT_API_KEY || 'test_fWTiPXsQgZVLfKlkuqfYnBRtuLG';
...
export async function purchaseProduct(identifier: string): Promise<boolean> {
  try {
    const Purchases = await getPurchases();
    // @ts-ignore — react-native-purchases type mismatch on purchaseProduct
...
export async function getPromotionalOffer(productIdentifier: string, discountIdentifier: string): Promise<unknown | null> {
  try {
    const Purchases = await getPurchases();
    // @ts-ignore — react-native-purchases type mismatch
...
export async function logIn(appUserId: string): Promise<unknown | null> {
  try {
    const Purchases = await getPurchases();
